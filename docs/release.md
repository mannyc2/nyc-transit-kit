...
The standard release commands resolve the `release` binary from
...
For local dogfooding of an unpublished `ts-release` checkout, set
`TS_RELEASE_CLI` to a built CLI path. Do not commit local `ts-release` paths,
